...
def run():
    ...
    run_time_data = np.zeros(y.shape[0])
    ...
    run_time_predata = np.loadtxt(str(name) +'_'+str(number)+ '_run_time.txt')
    try:
    ...
    for i in range(0, y.shape[0]):
        ...
        run_time_data[i] = run_time_predata[i]
    ...
    if not reduce_data:
        ...
        np.savetxt(str(name) +'_'+str(number)+ '_run_time.data', np.c_[y,run_time_data]) 
        try:
        ...
        old_run_time_data = np.c_[y,run_time_data]
        ...
        run_time_new_data = np.zeros(( int(run_time_data.shape[0]/reduction_number),2) )
        ...
        for i in range(0, v_new_data.shape[0]):
            ...
            try:
                ...
                run_time_new_data[i] = old_run_time_data[i*reduction_number]
        ...
        np.savetxt(str(name) +'_'+str(number)+ '_run_time.data', run_time_new_data)
        try:
